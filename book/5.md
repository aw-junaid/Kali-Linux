...
This chapter is organized as a toolkit reference. Each section corresponds to one widely used tool available on Kali Linux (or easily installed on it), and each tool is documented in its own dedicated markdown file so that it can be used as a standalone reference sheet during an engagement. The tools covered span several categories:
